...
class BrowserCopilot:
    ...
    def start(self, headless: Optional[bool] = None) -> "BrowserCopilot":
        ...
        if headless is not None:
            self.headless = headless
        try:
            print(f"[BrowserCopilot] Starting browser with self.proxy = {self.proxy}")
            ...
            if self.proxy:
                ...
                print(f"[BrowserCopilot] Playwright proxy config: {launch_kwargs['proxy']}")
    ...
    def login(self, path: str = DEFAULT_AUTH_FILE, timeout: int = 300) -> dict:
        ...
        # 发送首条消息以触发人机验证并等待用户完成
        if detected and not self._window_closed():
            self._page.wait_for_timeout(1500)
            log("sending first message to trigger captcha")
            print("\n[INFO] 正在自动发送首条消息以触发人机验证...")
            try:
                self._page.wait_for_selector("textarea#userInput", timeout=15000)
                self._page.fill("textarea#userInput", "Hello, who are you?")
                self._page.press("textarea#userInput", "Enter")
                log("first message sent successfully")
                print("[SUCCESS] 已自动发送消息 'Hello, who are you?'")
            except Exception as e:
                log(f"failed to auto-send message: {e}")
                print(f"[WARNING] 自动发送消息失败 ({e})，请在浏览器中手动输入消息并发送以触发人机验证。")

            print(
                "\n============================================================\n"
                " 请在浏览器窗口中完成登录和首轮消息带来的人机校验（如 Cloudflare Turnstile）。\n"
                " 当您在浏览器页面中看到 Copilot 正常回复后，请在当前终端按下 [Enter] 键保存凭证。\n"
                " (本工具不会自动关闭浏览器，以保证 Session 凭证的落盘保存成功)\n"
                "============================================================\n"
            )
            try:
                input("按下 [Enter] 键继续并保存凭证...")
        ...
        print(f"Session saved to {self.profile_dir} (Browser tab kept open)")
        return auth
    ...
    def cookies(self) -> Dict[str, str]:
        ...
        try:
            ...
            return {}
        return {c["name"]: c["value"] for c in raw if "microsoft.com" in c.get("domain", "") or "bing.com" in c.get("domain", "")}
